...
export class MapPage extends BasePage {
  ...
  getCompositionPreview(slug: string): Locator {
    return this.page.locator(`[data-map-composition-preview][aria-labelledby="map-composition-label-${slug}"]`)
  }
  ...
  async expectCompositionParity(fixture: MapCompositionFixture): Promise<void> {
    ...
    const layerIds = await this.viewport
      ...
      .evaluateAll((layers) => layers.map((layer) => layer.getAttribute('data-map-layer')))
  ...
  getSoundMark(markId: number): Locator {
    return this.page.locator(`[data-testid="sound-mark"][data-mark-id="${markId}"]`)
  }

  /** One sound button inside a mark's fan for a sound id. */
  getSoundButton(markId: number, soundId: number): Locator {
    return this.getSoundMark(markId).locator(`[data-testid="sound-button"][data-sound-id="${soundId}"]`)
  }
  ...
  /**
   * Pushes the map to its maximum zoom level (the engine clamps the scale
   * at `maxScale`). Clicking the zoom-in control converges in a few steps;
   * stops as soon as the scale stops growing.
   */
  async zoomToMax(maxClicks = 10): Promise<number> {
    const initial = await this.getZoom()
    let scale = initial
    for (let i = 0; i < maxClicks; i += 1) {
      await this.zoomInButton.click()
      const next = await this.getZoom()
      if (next <= scale) break
      scale = next
    }
    return scale
  ...
  /** Screen rectangles of every sound button (fit/max positions included). */
  async getSoundButtonBoxes(): Promise<Array<{ x: number; y: number; w: number; h: number }>> {
    return this.soundButtons.evaluateAll((els) =>
      els.map((el) => {
  ...
  /** Screen rectangle of the interactive viewport surface. */
  async getViewportBox(): Promise<{
    x: number
    y: number
    w: number
    h: number
  }> {
    const box = await this.page.locator('.map-viewport').boundingBox()
    if (box === null) throw new Error('Map viewport has no bounding box')
    return { x: box.x, y: box.y, w: box.width, h: box.height }
  }

  /** Installs gesture counters and returns a reset callback. */
  async installGestureCounters(): Promise<() => Promise<void>> {
    await this.page.evaluate(() => {
      ...
      const win = window as Window & {
        ...
        __countersInstalled?: boolean
      }
      if (win.__countersInstalled) return
      win.__countersInstalled = true
  ...
  async getSoundActivations(): Promise<number> {
    return this.page.evaluate(() => (window as Window & { __soundActivations?: number }).__soundActivations ?? 0)
  }

  async getViewportDrags(): Promise<number> {
    return this.page.evaluate(() => (window as Window & { __viewportDrags?: number }).__viewportDrags ?? 0)
  }
  ...
  async getMarkPosition(markId: number): Promise<MarkPositionProbe> {
    ...
    return mark.evaluate((element) => {
      ...
      const x = Number.parseFloat(element.style.getPropertyValue('--mark-x'))
      const y = Number.parseFloat(element.style.getPropertyValue('--mark-y'))
      return {
        x,
        y,
  ...
  async getBounds(): Promise<MapBounds> {
    return this.page.evaluate(() => {
      ...
      const image = document.querySelector('.map-world')?.getBoundingClientRect()
