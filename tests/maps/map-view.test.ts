...
async function waitForStatus(el: MapView, status: string, timeout = 1000): Promise<void> {
  const start = Date.now()
...
describe('MapView custom element', () => {
  ...
  it('creates the expected DOM structure and marks itself ready', async () => {
    ...
    expect(el.querySelector('.map-panzoom > .map-world > [data-map-layer] > img')).not.toBeNull()
  ...
  it('gives the base image a descriptive alt and keeps overlays decorative', async () => {
    ...
    expect(baseImg?.getAttribute('alt')).toBe('Mapa de Plaza de Armas - La Serena')
  ...
  it('hands off in degraded mode after an optional layer decode failure', async () => {
    HTMLImageElement.prototype.decode = function () {
      ...
      return this.src.includes('#overlay') ? Promise.reject(new Error('overlay failed')) : Promise.resolve()
    }
    const el = document.createElement('map-view') as MapView
    setLayers(el)
    const error = vi.fn()
  ...
  it('publishes error with an accessible diagnostic when the base image has invalid dimensions', async () => {
    ...
    expect(el.hasAttribute('data-ready')).toBe(false)
    expect(el.getAttribute('data-composition-status')).toBe('error')
    expect(el.querySelector('[role="alert"]')?.textContent).toContain('invalid dimensions')
    expect(error).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('publishes error when the viewport engine construction fails', async () => {
    ...
    expect(el.hasAttribute('data-ready')).toBe(false)
    expect(el.getAttribute('data-composition-status')).toBe('error')
    expect(el.querySelector('[role="alert"]')?.textContent).toContain('engine failed to initialize')
    expect(error).toHaveBeenCalledWith(
      expect.objectContaining({
        detail: expect.objectContaining({
  ...
  it('renders a data-driven hover scale on non-base layers only', async () => {
    ...
    const overlay = el.querySelector('[data-map-layer="overlay"]') as HTMLElement
  ...
  it('ignores a stale initializer failure after the element reconnects', async () => {
    ...
    HTMLImageElement.prototype.decode = function () {
      ...
      return new Promise<void>((resolve, reject) => decodes.push({ resolve, reject }))
    }
    const el = document.createElement('map-view') as MapView
    setLayers(el)
    wrapper.appendChild(el)
  ...
  it('throws when map-src is missing', () => {
    ...
    expect(() => wrapper.appendChild(el)).toThrow('<map-view> requires a "map-src" attribute')
  })
  ...
  it.each(['', '0', '-1', 'Infinity', 'NaN', '1px'])('throws for invalid zoom value "%s"', (value) => {
    const el = document.createElement('map-view') as MapView
    el.setAttribute('map-src', TEST_IMAGE_SRC)
    el.setAttribute('start-zoom', value)

    expect(() => wrapper.appendChild(el)).toThrow('<map-view> "start-zoom" attribute must be a finite positive number')
  })
  ...
  it('re-fits the world only on resize viewport-change reasons, never on gesture frames', async () => {
    ...
    expect(engineInstances[0]?.setViewportTransformScaleFactor).toHaveBeenCalledWith(0.5)
  })
