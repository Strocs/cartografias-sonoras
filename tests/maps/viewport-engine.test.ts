...
describe('ViewportEngine corrected interaction semantics', () => {
  ...
  it('resists active overscroll, snaps strictly, and publishes neither drag coast nor inertial reasons after release', () => {
    ...
    scene.addEventListener('viewport-change', (event) =>
      changes.push((event as CustomEvent<ViewportEventDetail>).detail)
    )
    const start = engine.getState()
    ...
    expect(changes.map(({ reason }) => reason)).not.toEqual(
      expect.arrayContaining([expect.stringMatching(/momentum|inertia|coast/i)])
    )
  ...
  it('preserves the focal content point at permitted min and max anchors and publishes one strict terminal wheel state', () => {
    ...
    scene.addEventListener('viewport-change', (event) =>
      details.push((event as CustomEvent<ViewportEventDetail>).detail)
    )
    ...
    const terminalPublications = details.filter(
      ({ reason, state }) => reason === 'wheel-settle' && state.phase === 'idle' && state.scale === config.maxScale
    )
  ...
  it('rebases rapid wheel bursts from the rendered state and latest cursor without jumps or concurrent loops', () => {
    ...
    expect(engine.getState()).toEqual(
      expect.objectContaining({ x: rendered.x, y: rendered.y, scale: rendered.scale, phase: rendered.phase })
    )
  ...
  it('keeps every wheel frame observably identical across DOM, snapshot, subscriber, event, revision, phase, and reason', () => {
    ...
    scene.addEventListener('viewport-change', (event) =>
      events.push((event as CustomEvent<ViewportEventDetail>).detail)
    )
  ...
  it('interactive pointer does not preventDefault; scene background pointer does', () => {
    ...
    const buttonDown = new PointerEvent('pointerdown', {
      bubbles: true,
      cancelable: true,
      pointerId: 1,
      clientX: 100,
    ...
    const sceneDown = new PointerEvent('pointerdown', {
      bubbles: true,
      cancelable: true,
      pointerId: 2,
      clientX: 200,
