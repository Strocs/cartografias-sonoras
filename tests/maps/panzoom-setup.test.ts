...
function bounds(content: TransformBoundsInput['content'], scale = 1, pan = { x: 0, y: 0 }): TransformBoundsInput {
...
describe('gesture coordinator', () => {
  ...
  it('constrains the requested pointer pan before Panzoom receives it and smooths back once on release', () => {
    ...
    document.dispatchEvent(new PointerEvent('pointerup', { bubbles: true, pointerId: 1, clientX: 2200, clientY: 150 }))
    expect(panzoomMock.pan).toHaveBeenLastCalledWith(100, 0, expect.objectContaining({ animate: true }))
  ...
  it('clamps scale to strict bounds on pinch release and constrains pan', () => {
    ...
    document.dispatchEvent(new PointerEvent('pointerup', { bubbles: true, pointerId: 1, clientX: 150, clientY: 150 }))
    document.dispatchEvent(new PointerEvent('pointerup', { bubbles: true, pointerId: 2, clientX: 250, clientY: 150 }))
