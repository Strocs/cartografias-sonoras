...
describe('checkRouteTopology — mock dataset', () => {
  it('satisfies N-1 for every mock map', () => {
    for (const map of mockMaps) {
      const result = checkRouteTopology(map.id, mapSounds(map.id), mapPaths(map.id));
...
describe('checkWaypointInvariants — finite geometry points', () => {
  it('accepts finite in-range waypoints', () => {
    const path: Path = { id: 1, mapId: 2, waypoints: [{ x: 50, y: 50 }], startSoundId: 201, endSoundId: 202 };
    expect(() => checkWaypointInvariants(path)).not.toThrow();
  });

  it('rejects a non-finite waypoint x', () => {
    const path: Path = { id: 1, mapId: 2, waypoints: [{ x: Infinity, y: 50 }], startSoundId: 201, endSoundId: 202 };
  ...
  it('rejects a non-finite waypoint y', () => {
    const path: Path = { id: 1, mapId: 2, waypoints: [{ x: 50, y: NaN }], startSoundId: 201, endSoundId: 202 };
    expect(() => checkWaypointInvariants(path)).toThrow(
      'Waypoint coordinates must be finite'
    );
  });
  ...
  it('rejects a waypoint outside the percentage range', () => {
    const path: Path = { id: 1, mapId: 2, waypoints: [{ x: 150, y: 50 }], startSoundId: 201, endSoundId: 202 };
    expect(() => checkWaypointInvariants(path)).toThrow(
  ...
  it('accepts an empty waypoint list', () => {
    const path: Path = { id: 1, mapId: 2, waypoints: [], startSoundId: 201, endSoundId: 202 };
    expect(() => checkWaypointInvariants(path)).not.toThrow();
  });
});
