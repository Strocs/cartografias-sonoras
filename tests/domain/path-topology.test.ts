...
import { checkRouteTopology, checkWaypointInvariants } from '../../src/features/paths/domain'
...
const mapMarks = (mapId: number): number[] => MARKS.filter((m) => m.mapId === mapId).map((m) => m.id)

const mapPaths = (mapId: number): Path[] => PATHS.filter((p) => p.mapId === mapId)

describe('checkRouteTopology — dataset', () => {
  it('satisfies N-1 for every map', () => {
    for (const map of MAPS_DATA) {
      const result = checkRouteTopology(map.id, mapMarks(map.id), mapPaths(map.id))
...
describe('checkRouteTopology — linear route invariants', () => {
  it('accepts a valid linear route', () => {
    const result = checkRouteTopology(2, [201, 205, 202, 203, 204], linearRoute())
    expect(result.valid).toBe(true)
    expect(result.errors).toEqual([])
  })
  ...
  it('rejects a self-connection', () => {
    const paths = [{ id: 1, mapId: 2, waypoints: [], startMarkId: 201, endMarkId: 201 }, ...linearRoute().slice(1)]
    const result = checkRouteTopology(2, [201, 205, 202, 203, 204], paths)
    expect(result.valid).toBe(false)
  ...
  it('rejects duplicate undirected edges', () => {
    const paths = [...linearRoute(), { id: 5, mapId: 2, waypoints: [], startMarkId: 203, endMarkId: 202 }]
    const result = checkRouteTopology(2, [201, 205, 202, 203, 204], paths)
    expect(result.valid).toBe(false)
  ...
  it('rejects a mark with no path', () => {
    const result = checkRouteTopology(2, [201, 205, 202, 203, 204, 999], linearRoute())
    expect(result.valid).toBe(false)
  ...
  it('rejects a mark that does not belong to the map', () => {
    ...
    const result = checkRouteTopology(2, [201, 205, 202, 203, 204], paths)
    expect(result.valid).toBe(false)
    expect(result.errors.join('\n')).toContain('references mark 999 outside map 2')
  })
...
describe('checkWaypointInvariants — finite geometry points', () => {
  ...
  it('rejects a non-finite waypoint x', () => {
    ...
    expect(() => checkWaypointInvariants(path)).toThrow('Waypoint coordinates must be finite')
  })
  ...
  it('rejects a waypoint outside the percentage range', () => {
    ...
    expect(() => checkWaypointInvariants(path)).toThrow('Waypoint must be within 0–100')
  })
