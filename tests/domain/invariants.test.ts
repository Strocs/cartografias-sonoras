...
import { checkPathInvariants, pathSchema } from '../../src/features/paths/domain'
import type { SoundPiece } from '../../src/features/sound-pieces/domain'
import { checkSoundPieceInvariants, soundPieceSchema } from '../../src/features/sound-pieces/domain'
import type { Mark, Sound } from '../../src/features/sounds/domain'
import { checkMarkInvariants, checkSoundInvariants, markSchema, soundSchema } from '../../src/features/sounds/domain'
...
describe('Map invariants', () => {
  ...
  it('fails when soundPieceId is null', () => {
    ...
    expect(() => checkMapInvariants(invalid)).toThrow('Map must have a sound piece')
  })
  ...
  it('rejects invalid layers and preview dimensions', () => {
    ...
    expect(mapSchema.safeParse({ ...validMap, images: [] }).success).toBe(false)
  })
})
...
describe('SoundPiece invariants', () => {
  ...
  it('fails when mapId is null', () => {
    ...
    expect(() => checkSoundPieceInvariants(invalid)).toThrow('Sound piece must belong to a map')
  })
...
describe('Mark invariants', () => {
  ...
  it('fails when mapId is null', () => {
    ...
    expect(() => checkMarkInvariants(invalid)).toThrow('Mark must belong to a map')
  })
  ...
  it('fails when position is not finite', () => {
    ...
    expect(() => checkMarkInvariants(invalid)).toThrow('Mark position must be finite')
  })

  it('fails when position is outside the 0–100 percentage range', () => {
    const invalid: Mark = { ...validMark, position: { x: 50, y: 150 } }
    expect(() => checkMarkInvariants(invalid)).toThrow('Mark position must be within 0–100')
  })

  it('fails when sounds is empty', () => {
    const invalid: Mark = { ...validMark, sounds: [] }
    expect(() => checkMarkInvariants(invalid)).toThrow('Mark must contain at least one sound')
  })
  ...
  it('fails when sound ids are duplicated', () => {
    ...
    expect(() => checkMarkInvariants(invalid)).toThrow('Mark sounds must have unique ids')
  })
...
describe('Sound invariants', () => {
  ...
  it('fails when id is not finite', () => {
    ...
    expect(() => checkSoundInvariants(invalid)).toThrow('Sound id must be a finite positive number')
  })
  ...
  it('fails when audioUrl is empty', () => {
    ...
    expect(() => checkSoundInvariants(invalid)).toThrow('Sound audioUrl must be a non-empty URL')
  })
...
describe('Path invariants', () => {
  ...
  it('fails when mapId is null', () => {
    ...
    expect(() => checkPathInvariants(invalid)).toThrow('Path must belong to a map')
  })
  ...
  it('fails when startMarkId equals endMarkId', () => {
    ...
    expect(() => checkPathInvariants(invalid)).toThrow('Path must connect to two different marks')
  })
...
describe('Dataset cross-reference validator', () => {
  ...
  it('fails when a path references a mark from another map', () => {
    ...
    expect(result.success).toBe(false)
    expect(result.errors).toContain('Mark 100 in path 1000 does not belong to map 1')
  })
