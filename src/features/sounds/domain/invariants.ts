...
export function checkMarkInvariants(mark: Mark): void {
  ...
  if (
    !Number.isFinite(mark.position.x) ||
    !Number.isFinite(mark.position.y)
  ) {
  ...
  if (
    ...
    throw new Error(
      'Mark position must be within 0–100 (percentage of map dimensions)'
    );
  }
