...
import { computeFanRadius, computeFanSlots, MARK_RADIUS, MARK_SIZE, SOUND_FAN_OVERLAP } from './fanGeometry'
 ...
 */
export function insertFanButton(fan: HTMLDivElement, slotIndex: number, button: HTMLButtonElement): void {
 ...
 */
export function updateMark(group: HTMLDivElement, state: { scaleFactor?: number; active?: boolean }): void {
...
function applyTransform(group: HTMLDivElement, x: number, y: number, scaleFactor: number): void {
...
function createTail(): SVGSVGElement {
  ...
  path.setAttribute('d', `M 0 0 H ${MARK_SIZE} L ${MARK_SIZE / 2} ${PIN_TAIL_HEIGHT} Z`)
  ...
  border.setAttribute('d', `M 0 0 L ${MARK_SIZE / 2} ${PIN_TAIL_HEIGHT} L ${MARK_SIZE} 0`)
