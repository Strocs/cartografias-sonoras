...
export function initPanzoom(
  ...
  const emitTransformEvent = (name: 'panzoomstart' | 'panzoomend' | 'panzoomzoom' | 'panzoomreset', originalEvent?: Event) => {
  ...
  const onWheel = (event: WheelEvent) => {
    ...
    }, targetScale, STRICT_BOUNDS_MODE, event);
  };
  ...
  const zoomAtViewportCenter = (factor: number) => {
    ...
    }, targetScale, STRICT_BOUNDS_MODE);
  };
