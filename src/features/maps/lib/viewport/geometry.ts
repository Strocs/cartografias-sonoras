...
export function applyBoundsResistance(state: ViewportState, bounds: ViewportBounds): ViewportState {
  assertState(state);
  return {
    x: state.x < bounds.minX ? bounds.minX + applyResistance(state.x - bounds.minX) : state.x > bounds.maxX ? bounds.maxX + applyResistance(state.x - bounds.maxX) : state.x,
    y: state.y < bounds.minY ? bounds.minY + applyResistance(state.y - bounds.minY) : state.y > bounds.maxY ? bounds.maxY + applyResistance(state.y - bounds.maxY) : state.y,
    scale: state.scale,
  };
}
