...
export function createSnapBack(from: ViewportState, to: ViewportState, reducedMotion: boolean): ViewportTransition {
  assertState(from);
  assertState(to);
  return { from: { ...from }, to: { ...to }, durationMs: reducedMotion ? 0 : SNAP_BACK_DURATION_MS, easing: SNAP_BACK_EASING };
}
...
export function isReducedMotion(mediaQuery = '(prefers-reduced-motion: reduce)'): boolean {
  return typeof window !== 'undefined' && typeof window.matchMedia === 'function' && window.matchMedia(mediaQuery).matches;
}
