...
>(dataset: Dataset<M, Mk, P, Pa>): ValidationResult {
  ...
  for (const map of dataset.maps) {
    ...
    if (!piece) {
      errors.push(`Map ${map.id} references missing sound piece ${map.soundPieceId}`)
  ...
  for (const piece of dataset.soundPieces) {
    ...
    if (!map) {
      errors.push(`Sound piece ${piece.id} references missing map ${piece.mapId}`)
    }
  }
  ...
  for (const path of dataset.paths) {
    ...
    for (const markId of [path.startMarkId, path.endMarkId]) {
      ...
      if (!mark) {
        ...
        errors.push(`Mark ${markId} in path ${path.id} does not belong to map ${path.mapId}`)
      }
    }
  }
