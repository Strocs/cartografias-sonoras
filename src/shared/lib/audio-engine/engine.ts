import { AUDIO_STATUS, type AudioEngineState, type AudioStatus, type PieceState, type SoundState } from './types'

export { AUDIO_STATUS } from './types'
export type { AudioEngineState, AudioStatus, PieceState, SoundState } from './types'
...
export function playSound(state: AudioEngineState, soundId: number, mapId: number): AudioEngineState {
  ...
  if (
    ...
    (state.piece.status === AUDIO_STATUS.PLAYING || state.piece.status === AUDIO_STATUS.LOADING)
  ) {
    return state
  }
